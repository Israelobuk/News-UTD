...
function DashboardApp({ onNavigateHome }) {
  ...
  const allowSnapshotHydrationRef = useRef(true);
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (message?.type === "hello") {
            ...
            if (allowSnapshotHydrationRef.current) {
          ...
          const dedupeKey = nextAlert.postId || nextAlert.id;
          ...
          setQueuedAlerts((current) => [nextAlert, ...current]);
        } catch {
          setStreamError("Signal stream temporarily unavailable.");
        }
      };
  ...
  useEffect(() => {
    ...
    if (!activeAlertRef.current && allowSnapshotHydrationRef.current) {
  ...
  useEffect(() => {
    ...
    const syncWatchlist = async () => {
      allowSnapshotHydrationRef.current = true;
      setActiveAlert(null);
      setQueuedAlerts([]);
      setAlerts([]);
      setRecentAlerts([]);
  ...
  const activeAlertCount = filteredActiveAlert ? 1 : 0;
  const totalVisibleAlertCount = filteredAlertPoolCount + activeAlertCount;
  const totalFilterableAlertCount = feedPoolAlerts.length + (activeAlert ? 1 : 0);
  ...
  const handleResetFilters = useCallback(() => {
    const knownAlerts = mergeUniqueAlerts(
    ...
    for (const alert of knownAlerts) {
      const dedupeKey = alert.postId || alert.id;
      if (dedupeKey) {
        nextSeen.add(dedupeKey);
      }
    }

    allowSnapshotHydrationRef.current = false;
    ...
    <div className="dashboard-shell">
      <div className="dashboard">
        <Sidebar
          ...
          filterResultCount={totalVisibleAlertCount}
          filterTotalCount={totalFilterableAlertCount}
        />
